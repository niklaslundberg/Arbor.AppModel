...
public class MediatorRegistrationTests(ITestOutputHelper testOutputHelper)
{
    [Fact]
    public async Task NotificationsAndRequestsShouldBeHandledOnlyOncePerConcreteType()
    {
        ...
        {
            testOutputHelper.WriteLine($"Notification: {testHandlerInvokedNotification}");
        }

        foreach (var request in testHandler1.InvokedRequests)
        {
            testOutputHelper.WriteLine($"Request: {request}");
        }
